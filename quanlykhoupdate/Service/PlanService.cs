    ...
    public class PlanService : IPlanService
    {
        ...
        public async Task<PayLoad<object>> FindAllDataByDone(searchDataPost datas)
        {
            try
            {
                ...
                DateTime dateTo = datas.dateto?.UtcDateTime.Date.AddHours(2) ?? DateTime.UtcNow.Date.AddHours(8);
        ...
        public async Task<PayLoad<object>> FindAllDataByNoDone(searchDataPost datas)
        {
            try
            {
                ...
                DateTimeOffset dateToUtc = datas.dateto.Value.ToUniversalTime().AddHours(2); // Lấy hết ngày
